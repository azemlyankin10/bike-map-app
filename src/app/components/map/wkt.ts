...
let coordFormat = new RegExp(
  "^" + numberRegexp.source + "(\\s" + numberRegexp.source + "){1,}"
);

export const parse = (wkt: any) => {
  let parts = wkt.split(";");
  let _ = parts.pop() as any
  let srid = (parts.shift() || "").split("=").pop() as any
  ...
  function multicoords() {
    ...
    while ((elem = $(/^(\()/) || $(/^(\))/) || $(/^(,)/) || $(coordFormat))) {
      if (elem === "(") {
        ...
        pointer = [];
        stack[stack.length - 1].push(pointer);
      } else if (!elem.split(/\s/g).some(isNaN as any)) {
  ...
  function multipoint() {
    ...
    white();
    let newCoordsFormat = _.substring(_.indexOf("(") + 1, _.length - 1)
      .replace(/\(/g, "")
      .replace(/\)/g, "");
  ...
  function geometrycollection():any {
    let geometries = [];
    let geometry;
 ...
 */
export const stringify = (geoJSON:any) => {
  ...
  switch (gJ.type) {
    ...
    case "GeometryCollection":
      return (
        "GEOMETRYCOLLECTION (" + gJ.geometries.map(stringify).join(", ") + ")"
      );
